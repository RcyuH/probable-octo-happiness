...
from verl.experimental.dataset.tree_engine import PGTreeEngine, TreeEngine
...
class RLHFDataset(Dataset):
    ...
    def _add_system_prompt_to_doc(self, doc: dict):
        ...
        doc['prompt'].insert(0, {"role": "system", "content": system_prompt})
...
class TreeDataset(RLHFDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.config.sampler.tree_sampler.name == 'pg':
            self.engine:TreeEngine = PGTreeEngine.remote(len(self.dataframe), self.config)
        else:
            raise ValueError(f"Unknown tree sampler name: {self.config.sampler.tree_sampler.name}")
    ...
    def __getitem__(self, item):
        ...
        node = ray.get(self.engine.get_node.remote(item))
        original_item = ray.get(self.engine.get_original_ancestor_item.remote(item))
